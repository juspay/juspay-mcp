...
async def get_gateway_details(payload: dict, meta_info: dict = None) -> dict:
    ...
    logger.info(f"[DEBUG] get_gateway_details - Incoming payload: {payload}")
    logger.info(f"[DEBUG] get_gateway_details - Incoming meta_info: {meta_info}")
    ...
    if meta_info:
        token_response = meta_info.get("token_response", {})
        mid_from_meta = token_response.get("merchantId") or meta_info.get("merchantId")
    logger.info(f"[DEBUG] get_gateway_details - merchantId from meta_info: {mid_from_meta}")
    logger.info(f"[DEBUG] get_gateway_details - token_response: {meta_info.get('token_response', {}) if meta_info else 'None'}")
