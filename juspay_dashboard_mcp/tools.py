...
IMPORTANT: If unsure about the type of a provided ID, the agent should ask the user for clarification, providing the available ID fields (epg_txn_id, txn_uuid, pgr_rrn) as options to choose from.""",
        model=api_schema.orders.FindOrdersPayload,
        handler=orders.find_orders,
        response_schema=None,
    ),
    util.make_api_config(
...
- Render invoice_url as a short clickable [Download](url) link. Do not paste the full URL.""",
        model=api_schema.zoho_books.ListZohoBooksInvoicesPayload,
        handler=zoho_books.list_zoho_books_invoices,
        response_schema=None,
    ),
    util.make_api_config(
...
Use this tool when asked to look up details of a mandate by its mandate ID.""",
        model=api_schema.mandate.GetMandateDetailsPayload,
        handler=mandate.get_mandate_details,
...
Use this tool when asked about clients, client configuration, client config, platform integration status, enabled payment features, or payment subscription settings for a merchant.""",
        model=api_schema.clientConfigs.GetClientConfigPayload,
        handler=clientConfigs.get_client_config,
